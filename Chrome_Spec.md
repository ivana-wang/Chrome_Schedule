...
| 6 | FSI Sign-off: date（**= RTM**） | 引擎計算（`FSI qual complete + 3 days`，見 §6） |
...
| **FSI qual complete** | FSI Candidate + 1 week | D + 10 days |
| **FSI Sign-off（= RTM）** | FSI qual complete + 3 days | D + 13 days |

### 6.1 往後（forward，D 之後）

```
FSI Candidate = CFZ   = D + 3 days
FSI qual complete     = FSI Candidate + 7 days   (= CFZ + 1 week)
FSI Sign-off  = RTM   = FSI qual complete + 3 days   (= CFZ + 10 days, 規則日)
```

> **CFZ / RTM 落在週五**：算出「規則日」後 snap 到 **最早（當天或之前）的週五**（遇假日往前一個工作日）。
> **Pull-in 規則（紅字）**：當 snap 後的週五**早於規則日**，代表 sign-off 比規則允許的還早 →
> 視為 **pull-in opportunity**，UI 跳**紅字**提示「需額外溝通確認」。
> 例（golden D=8/25）：CFZ 規則日 = 8/28（週五，無 pull-in）；RTM 規則日 = CFZ+10 = **9/7（週一）**
> → 最早的週五 = **9/4**，比規則早 **3 天** → **pull-in，紅字**；若不 pull-in，RTM 落在 9/7。
...
> - **golden RTM 維持 9/4**：規則日 = CFZ+10 = 9/7，snap 到最早週五 = **9/4**（pull-in 3 天 → 紅字提示）。
