...
1. **錨點 = Stable Release date**（從上傳圖片讀出，本例 = Chrome Browser 8/25）。
2. 由 Stable Release **往前回推**每一個 TASK 的日期，一路回推到 **專案起跑日 1/6**
   （= `ID Master/Detail Ready` / `ME design` / `DB Layout` 的起點；**不是** sheet 的 `Kick off` 2/4）。
...
## 6. 向後推算：MV → FCS 的 ChromeBook 公式

從 Stable Release 往後，固定關係如下（已用 Example.png 的 8/25→8/28→9/4 驗證一致）：

```
FSI Candidate = CFZ = Stable Release + 3 days
RTM           = CFZ + 7 days
```

- `Stable Release + 3 days` = **FSI Candidate** = **CFZ**
- `CFZ + 7 days` = **RTM**（對應 Example.png 的 "FSI sign-off"）

> ✅ **已確認（用 xlsx D/E 欄資料驗證）：`+3` / `+7` 為 calendar days，結果再 snap 到週五。**
> 8/25(Tue) +3 cal = 8/28(Fri) = CFZ；8/28 +7 cal = 9/4(Fri) = RTM。完全等同 PRODUCT_SPEC 的
> CFZ/RTM 規則：**calendar +N → 套用 `fridayOrPrevWorkday`（落週五，遇假日往前一個工作日，不往後推一週）**。
> 直接沿用 PRODUCT_SPEC 的 `cfzFromPld` / `rtmFromCfz` 邏輯，只是錨點換成 Stable Release。
...
| RTM Ties-1 | 9/4 | System RTM | CFZ + 7 cal → 週五 |
| MV Pre-Build | 9/5 | MV Pre-Build | **MAX(MV SMT end+1, RTM+1)** = MAX(9/3, 9/5) = 9/5 ✓ |
